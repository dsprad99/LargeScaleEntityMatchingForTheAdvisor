...
from Kmer import query_selector,mer_hashtable, histogramQuery,histogramMers,query_selector_MAG_test, remove_top_k_mers, repeating_kmer_study,histogramRepeatedMers
import os, psutil
...
def main3():
    ...
    dblp_callbacks = [
        ...
        lambda current_paper: repeating_kmer_study(repeat_kmer_hashmap,current_paper.title, 3)
    ]
    num_papers = parse_DBLP_file(file_path_dblp, dblp_callbacks,100000000)
    #print(len(dblp_mer_hash.keys()))
    #print("Number of repeated mers:",repeat_kmer_hashmap)

    histogramRepeatedMers(repeat_kmer_hashmap,0,20, 'top_repeated_kmers.png')
